...
static void __always_inline parse_kern_evt_body(struct parse_kern_evt_body *param) {
	...
	evt->ts_delta = 0;
	evt->step = step;
	bpf_probe_read(evt->func_name,FUNC_NAME_LIMIT, func_name);
...
static void __always_inline parse_sock_key_from_ipv4_tcp_hdr(struct sock_key *key, struct iphdr *ipv4, 
	...
	u32 saddr = 0;
	u32 daddr = 0;
	BPF_CORE_READ_INTO(&saddr, ipv4, saddr);	//ipv3->saddr => saddr
	BPF_CORE_READ_INTO(&daddr, ipv4, daddr);
	u16 sport = 0;
	u16 dport = 0;
	BPF_CORE_READ_INTO(&sport, tcp, source);	//tcp->source => sport
...
static __always_inline int parse_skb(void* ctx, struct sk_buff *skb, bool sk_not_ready, enum step_t step) {
	...
	/*
	<idle>-0       [000] ..s31 12644415.790459: bpf_trace_printk: network_header=78, mac_header=64
    <idle>-0       [000] ..s31 12644415.790460: bpf_trace_printk: trans_header=98, step=9
	*/
	bpf_printk("network_header=%d, mac_header=%d",network_header, mac_header);
	bpf_printk("trans_header=%d, step=%d",trans_header, step);
	...
	void* ip = data + network_header;	//通常使用这种方式来定位到IP头 
	void *l3 = {0};
	void* l4 = NULL;
	u16 l3_proto;
	...
	if (is_l2) {
		...
		bpf_printk("isl2 true network_header=%d, mac_header=%d,goto __l2",network_header, mac_header);
		goto __l2;
	} else {
		u16 _protocol = 0;
		...
		if (l3_proto == ETH_P_IP || l3_proto == ETH_P_IPV6) {
			bpf_printk("%s, is_ip: %d goto __l3", "parse_skb", 1);
			l3 = data + network_header;
			goto __l3;
		}
	...
	__l3:	
			if (!skb_l4_check(trans_header, network_header)) {
				// 存在l4
				bpf_printk("%s, skb_l4_check: %d","parse_skb", 0);
			...
			if (l3_proto == ETH_P_IP)	//deal ipv4
			{
				...
				l4 = l4 ? l4 : ip + ip_hdr_len;	//优先使用trans_header的值，其次使用 ip+ip_hdr_len
				BPF_CORE_READ_INTO(&proto_l4, ipv4, protocol);		//协议
			...
			if (proto_l4 == IPPROTO_TCP) {	//deal tcp layer
				if (!inital_seq) {
					...
					if (found == NULL) {
						if (step == DEV_IN) {
							...
							bpf_printk("parse_skb step=DEV_IN update seq,inital_seq=%d",inital_seq);
							bpf_map_update_elem(&sock_xmit_map, &key, &inital_seq, BPF_NOEXIST);
						} else {
							bpf_printk("parse_skb step is not DEV_IN update seq[err],step=%d",step);
							goto err;
						}
						// goto err;
					} else {
						bpf_probe_read_kernel(&inital_seq, sizeof(inital_seq), found);
						bpf_printk("parse_skb step found sock_xmit_map seq,inital_seq=%d",inital_seq);
					}
				}else{
				...
				struct net_device *dev = _C(skb, dev);	//skb->dev
				if (dev) {
					body.ifindex = _C(dev, ifindex);	//dev->ifindex
				} else {
				...
				bpf_printk("%s, not match: %d", "parse_skb", _C(ipv4,saddr));
			}
		}
	}
	err:return BPF_OK;
...
int BPF_KPROBE(skb_copy_datagram_iter, struct sk_buff *skb, int offset, struct iov_iter *to, int len) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/__skb_datagram_iter: pid=%d",ppppppid);
...
int BPF_KPROBE(skb_copy_datagram_iovec, struct sk_buff *skb, int offset, struct iov_iter *to, int len) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/skb_copy_datagram_iovec pid=%d",ppppppid);
	return handle_skb_data_copy(ctx, skb, offset, to, len);
}
...
int tracepoint__skb_copy_datagram_iovec(struct trace_event_raw_skb_copy_datagram_iovec  *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/skb/skb_copy_datagram_iovec pid=%d",ppppppid);
...
int tracepoint__netif_receive_skb(struct trace_event_raw_net_dev_template  *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/net/netif_receive_skb pid=%d",ppppppid);
	void *p = (void*)ctx + sizeof(struct trace_entry);
	struct sk_buff *skb;
	bpf_probe_read_kernel(&skb, sizeof(struct sk_buff *), p);
...
int BPF_KPROBE(tcp_queue_rcv, struct sock *sk, struct sk_buff *skb) {
	...
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/tcp_queue_rcv pid=%d",ppppppid);
	return BPF_OK;
} 
...
int BPF_KPROBE(tcp_rcv_established, struct sock *sk, struct sk_buff *skb) {
	...
	bpf_printk("kprobe/tcp_rcv_established pid=%d",ppppppid);
	return BPF_OK;
}  
...
int BPF_KPROBE(tcp_v4_do_rcv, struct sock *sk, struct sk_buff *skb) { 
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/tcp_v4_do_rcv pid=%d",ppppppid);
...
int BPF_KPROBE(tcp_v6_do_rcv, struct sock *sk, struct sk_buff *skb) { 
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/tcp_v6_do_rcv pid=%d",ppppppid);
	parse_skb(ctx, skb, 1, TCP_IN);
	return BPF_OK;
}
...
int BPF_KPROBE(tcp_v4_rcv, struct sk_buff *skb) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/tcp_v4_rcv pid=%d",ppppppid);
...
int BPF_KPROBE(ip_rcv_core, struct sk_buff *skb) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/ip_rcv_core pid=%d",ppppppid);
...
int BPF_KPROBE(dev_hard_start_xmit, struct sk_buff *first) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/dev_hard_start_xmit pid=%d",ppppppid);
...
int BPF_KPROBE(dev_queue_xmit, struct sk_buff *skb) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/dev_queue_xmit pid=%d",ppppppid);
...
{
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/__ip_queue_xmit pid=%d",ppppppid);
...
{
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("raw_tp/tcp_destroy_sock pid=%d",ppppppid);
	struct sock_key key = {0};
	// parse_sock_key_sk(sk, &key);
...
int BPF_KPROBE(kprobe__nf_nat_packet, struct nf_conn *ct) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/nf_nat_packet pid=%d",ppppppid);
    handle_nat(ct);
    return 0;
}
...
int BPF_KPROBE(kprobe__nf_nat_manip_pkt, void *_, struct nf_conn *ct) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/nf_nat_manip_pkt pid=%d",ppppppid);
    handle_nat(ct);
    return 0;
}
...
static __always_inline void process_syscall_accept(void* ctx, long int ret, struct accept_args *args, uint64_t id) {
	uint32_t tgid = id >> 32;
	if (ret < 0) {
		 bpf_printk("process_syscall_accept, ret_fd: %d, socket:%d", -ret,args->sock_alloc_socket);
		return;
	}
	...
	if (match_trace_tgid(tgid) == TARGET_TGID_UNMATCHED) {
	 ...
	 bpf_printk("process_syscall_accept, tgid:%lu, ret_fd: %d, socket:%d", tgid,ret,args->sock_alloc_socket);
...
static __always_inline bool create_conn_info_in_data_syscall(void* ctx, struct tcp_sock* tcp_sk,uint64_t tgid_fd, enum traffic_direction_t direct,ssize_t bytes_count,
		...
		if (direct == kEgress) {
			...
			rcv_initial_seq = copied_seq - 1;
			 bpf_printk("send initial_seq: %u", send_initial_seq); 
		} else {
			send_initial_seq = write_seq - 1;
			rcv_initial_seq = copied_seq - bytes_count - 1;
			 bpf_printk("recv initial_seq: %u", send_initial_seq);
		}
...
int BPF_KPROBE(security_socket_sendmsg_enter) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/security_socket_sendmsg pid=%d",ppppppid);
...
int BPF_KPROBE(security_socket_recvmsg_enter) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kprobe/security_socket_recvmsg pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_enter_recvfrom(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_recvfrom pid=%d",ppppppid);
	int flags = 0;
	TP_ARGS(&flags, 3, ctx)
...
int tracepoint__syscalls__sys_exit_recvfrom(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_recvfrom pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	ssize_t bytes_count ;
...
int tracepoint__syscalls__sys_enter_read(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_read pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_exit_read(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_read pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	ssize_t bytes_count;
	TP_RET(&bytes_count, ctx)
	struct data_args *args = bpf_map_lookup_elem(&read_args_map, &id);
...
int tracepoint__syscalls__sys_enter_recvmsg(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_recvmsg pid=%d",ppppppid);
	int flags = 0;
	TP_ARGS(&flags, 2, ctx)
	if ((flags & MSG_OOB) ||(flags & MSG_PEEK)) {
		return 0;
...
int tracepoint__syscalls__sys_exit_recvmsg(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_recvmsg pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	ssize_t bytes_count;
	TP_RET(&bytes_count, ctx)
...
int tracepoint__syscalls__sys_enter_readv(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_readv pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_exit_readv(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_readv pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	ssize_t bytes_count ;
	TP_RET(&bytes_count, ctx)
	struct data_args *args = bpf_map_lookup_elem(&read_args_map, &id);
...
int tracepoint__syscalls__sys_enter_sendfile64(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_sendfile64 pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_exit_sendfile64(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_sendfile64 pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	ssize_t bytes_count;
	TP_RET(&bytes_count, ctx)
...
int tracepoint__syscalls__sys_enter_sendto(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_sendto pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_exit_sendto(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_sendto pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	ssize_t bytes_count;
	TP_RET(&bytes_count, ctx)
...
int tracepoint__syscalls__sys_enter_write(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_write pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_exit_write(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_write pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	ssize_t bytes_count;
	TP_RET(&bytes_count, ctx)
...
int tracepoint__syscalls__sys_enter_sendmsg(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_sendmsg pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	struct my_user_msghdr* msghdr;
	TP_ARGS(&msghdr, 1, ctx)
	int sockfd ; 
...
int tracepoint__syscalls__sys_exit_sendmsg(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_sendmsg pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	ssize_t bytes_count;
	TP_RET(&bytes_count, ctx)
...
int tracepoint__syscalls__sys_enter_writev(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_writev pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_exit_writev(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_writev pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	ssize_t bytes_count;
	TP_RET(&bytes_count, ctx)
...
int tracepoint__syscalls__sys_enter_close(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_close pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
{
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_close pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_enter_connect(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_connect pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_exit_connect(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_connect pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_enter_accept4(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_accept4 pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_enter_accept(struct trace_event_raw_sys_enter *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_enter_accept pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
...
{
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("kretprobe/sock_alloc");
	uint64_t id = bpf_get_current_pid_tgid();
...
int tracepoint__syscalls__sys_exit_accept(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_accept pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	struct accept_args *args = bpf_map_lookup_elem(&accept_args_map, &id);
...
int tracepoint__syscalls__sys_exit_accept4(struct trace_event_raw_sys_exit *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/syscalls/sys_exit_accept4 pid=%d",ppppppid);
	uint64_t id = bpf_get_current_pid_tgid();
	struct accept_args *args = bpf_map_lookup_elem(&accept_args_map, &id);
...
int tracepoint__sched__sched_process_exec(struct trace_event_raw_sched_process_exec *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/sched/sched_process_exec pid=%d",ppppppid);
...
int tracepoint__sched__sched_process_exit(struct trace_event_raw_sched_process_exec *ctx) {
	int ppppppid = bpf_get_current_pid_tgid() >> 32;
	bpf_printk("tracepoint/sched/sched_process_exit pid=%d",ppppppid);
